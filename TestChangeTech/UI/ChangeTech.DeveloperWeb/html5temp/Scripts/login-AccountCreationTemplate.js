...
function loadDataForLogin(pageObj) {
    ...
    for (var i = 1; i < textArray.length; i++) {
        if (textArray[i] != "" && textArray[i] != undefined && textArray[i] != null) {
            loginStr += '<div class="row"><span class="label">' + textArray[i] + '</span><span class="formw"><input class="textfield loginEmail" type="text" size="25" autofocus /></span></div>';
        }
    }
    loginStr += '</form>';
...
function loadDataForPinCode(pageObj) {
    ...
    if (textArray[1] != "" && textArray[1] != undefined && textArray[1] != null) {
        pinCodeStr += '<div class="row"><span class="label">' + textArray[1] + '</span><span class="formw"><input class="textfield loginEmail" type="text" size="25" autofocus /></span></div>';
    }
    pinCodeStr += '</form>';
    $("#yellow").append(pinCodeStr);
    if (textArray[2] != "" && textArray[2] != undefined && textArray[2] != null) {
        $('#yellow ').append('<p id="bubble-pincode"><a href="javascript:void(0)" title="' + textArray[2] + '" class="' + pageType + '">' + textArray[2] + '</a></p>');
    }
}
//*******************
...
function onClickForPinCodeReminder(pageObj) {
    var pageType = pageObj.attr("Type");
    if (pageType == "PinCode") {
        $("#bubble-pincode a").click(function () {
