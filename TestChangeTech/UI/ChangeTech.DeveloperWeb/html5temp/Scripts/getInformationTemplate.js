...
function loadDataForSlider(quesIndex, ques) {
    ...
    var sliderItem = "";
    var sliderStr =
				   '<div class="sliders sliders' + quesIndex + '"><fieldset><div>' +
		           ...
		           "<input type='text' id='amount" + quesIndex + "' class='sliderFeedback feedbackLabel' /><input type='text' class='hiddenGuid' id='itemguid" + quesIndex + "' name='itemguid" + quesIndex + "' value='0'/></fieldset></div>";
    $('#yellow').append(sliderStr);
    if (captionInsteadRoundStr != null && captionInsteadRoundStr != 'undefined' && captionInsteadRoundStr != '') {
        $('#yellow .sliders' + quesIndex + ' fieldset').prepend('<legend>' + captionInsteadRoundStr + '</legend>');
    }
...
function loadDataForDropdownList(quesIndex, ques) {
    ...
    if (itemsObj != null && itemsObj != '' && itemsObj != undefined) {
        ...
        if (itemObj != null && itemsObj != '' && itemsObj != undefined) {
            $(itemObj).each(function (itemIndex, item) {
                ...
                if (feedbackValue != null && feedbackValue != '' && feedbackValue != undefined) {
                    ...
                    if (startIndex >= 0 && endIndex >= 0) {
                        optionValue = feedbackValueInsteadRoundStr.substring(startIndex + 3, endIndex);
                    }
                }
            ...
            var dropdownListStr = '<div class="dropdownList dropdownList' + quesIndex + '"><fieldset>' +
	               '<span class="formw">' +
            ...
            if (captionInsteadRoundStr != null && captionInsteadRoundStr != 'undefined' && captionInsteadRoundStr != '') {
                $('#yellow .dropdownList' + quesIndex + ' fieldset').prepend('<legend>' + captionInsteadRoundStr + '</legend>');
            }
            var defaultFeedback = "";
            var answer = "";
            if (retakeValue == "true" || retakeValue == "1") {
                ...
                if ($(ques).attr("Type") == "DropDownList") {
                    $('select[name=dropdownlistGroup' + quesIndex + ']').change(function () {
                        ...
                        if (feedback != 'undefined' && feedback != '' && feedback != null) {
                            $('.dropDownListfeedback' + quesIndex + '').text(feedback);
                        }
                        itemScore[quesGUID] = scoreValue;
                        GUIDValue[quesGUID] = itemGUID;
                    });
...
function loadDataForCheckbox(quesIndex, ques) {
    ...
    var checkBoxStr = "<div class='check check" + quesIndex + "'><fieldset><div class='checkboxrow" + quesIndex + "'></div></fieldset></div>";
    var itemsObj = $(ques).find("Items");
    if (itemsObj != null && itemsObj != undefined && itemsObj != '') {
        var itemObj = itemsObj.find("Item");
        if (itemObj != '' && itemObj != undefined && itemObj != null) {
            $(itemObj).each(function (itemIndex, item) {
                ...
                if (itemValueInsteadRoundStr != null && itemValueInsteadRoundStr != undefined && itemValueInsteadRoundStr != '') {
                    checkbox += '<div class="itemRow"><input class="checkbox" type="checkbox" name="checkboxGroup' + quesIndex + '" value="' + nonQuotFeedbackValue + ';' + scoreValue + ';' + itemGUID + '"><label for="check-' + itemIndex + '">' + itemValueInsteadRoundStr + '</label></div>';
                }
            });
            ...
            if (captionInsteadRoundStr != null && captionInsteadRoundStr != 'undefined' && captionInsteadRoundStr != '') {
                $('#yellow .check' + quesIndex + ' fieldset').prepend('<legend>' + captionInsteadRoundStr + '</legend>');
            }
        }
    }
    if (retakeValue == "true" || retakeValue == "1") {
                ...
                $(s).change(function () {
                    if ($(s).attr("checked")) {
                        if (feedback != '' && feedback != 'undefined' && feedback != null) {
                            ...
                            $("#bubble-yellow").css('height', height + 20 + 'px');
                        }
                    } else {
    ...
    if (itemsObj != null && itemsObj != undefined) {
        ...
        if (itemObj != null && itemObj != undefined) {
            $(itemObj).each(function (itemIndex, item) {
                ...
                radioButton += '<div class="itemRow"><input class="radioButton" type="radio" name="radioButtonGroup' + quesIndex + '" id="radio-' + itemIndex + '" value="' + nonQuotFeedbackValue + ';' + scoreValue + ';' + itemGUID + '"><label for="radio-' + itemIndex + '">' + itemInsteadRoundStr + '</label></div>';
            });
            var radioButtonStr = "<div class='radios radios" + quesIndex + "'><fieldset><div class='radioButtonRow" + quesIndex + "'></div></fieldset></div>";
            ...
            if (captionInsteadRoundStr != null && captionInsteadRoundStr != 'undefined' && captionInsteadRoundStr != '') {
                $('#yellow .radios' + quesIndex + ' fieldset').prepend('<legend>' + captionInsteadRoundStr + '</legend>');
            }
        }
    }
    if (retakeValue == "true" || retakeValue == "1") {
...
function loadDataForMultiLine(quesIndex, ques) {
    ...
    var multiplyStr = "<div class='textarea textarea" + quesIndex + "'><fieldset><textarea rows='5' autofocus='true' class='textfield textareaField'/></fieldset></div>";
    $('#yellow').append(multiplyStr);
    if (answer != '' && answer != "undefined" && answer != '') {
        saveAnswersForMultipline(answer, quesIndex, quesGUID);
    }
    if (captionInsteadRoundStr != null && captionInsteadRoundStr != 'undefined' && captionInsteadRoundStr != '') {
        $('#yellow .textarea' + quesIndex + ' fieldset').prepend('<legend>' + captionInsteadRoundStr + '</legend>');
    }
    if (retakeValue == "true" || retakeValue == "1") {
        if (quesprogramVariableName == undefined || quesprogramVariableName == null && quesprogramVariableName == '') {
...
function loadDataForSingleLine(quesIndex, ques) {
    ...
    var singlelineStr = "<div class='singleLine singleLine" + quesIndex + "'><fieldset><input type='text'  autofocus='true' class='textfield'/></fieldset></div>";
    $('#yellow').append(singlelineStr);
    if (answer != '' && answer != "undefined" && answer != null) {
        saveAnswersForSingleline(answer, quesIndex, quesGUID);
    }
    if (captionInsteadRoundStr != null && captionInsteadRoundStr != 'undefined' && captionInsteadRoundStr != '') {
        $('#yellow .singleLine' + quesIndex + ' fieldset').prepend('<legend>' + captionInsteadRoundStr + '</legend>');
    }
    if (retakeValue == "true" || retakeValue == "1") {
        if (quesprogramVariableName == undefined || quesprogramVariableName == null && quesprogramVariableName == '') {
...
function loadDataForHiddenSingleLine(quesIndex, ques) {
    ...
    var hiddensinglelineStr = "<div class='hiddenSingleLine hiddenSingleLine" + quesIndex + "'><fieldset><input type='password'  autofocus='true' class='textfield'/></fieldset></div>";
    $('#yellow').append(hiddensinglelineStr);
    if (answer != '' && answer != "undefined" && answer != null) {
        saveAnswersForHiddenSingleline(answer, quesIndex, quesGUID);
    }
    if (captionInsteadRoundStr != null && captionInsteadRoundStr != 'undefined' && captionInsteadRoundStr != '') {
        $('#yellow .hiddenSingleLine' + quesIndex + ' fieldset').prepend('<legend>' + captionInsteadRoundStr + '</legend>');
    }
    if (retakeValue == "true" || retakeValue == "1") {
        if (quesprogramVariableName == undefined || quesprogramVariableName == null && quesprogramVariableName == '') {
...
function loadDataForNumeric(quesIndex, ques) {
    ...
    var numericStr = '<div class="qbox qbox' + quesIndex + '"><fieldset>' +
                   '<div class="qinput">' +
    ...
    if (captionInsteadRoundStr != null && captionInsteadRoundStr != 'undefined' && captionInsteadRoundStr != '') {
        $('#yellow .qbox' + quesIndex + ' fieldset').prepend('<legend>' + captionInsteadRoundStr + '</legend>');
        /*if(answer == ''){
...
function showIllustrationImage(mediaType, mediaName) {
    var mediaUrl = "";
    if (mediaType == "Illustration" && mediaType != null && mediaType != undefined) {
        if (mediaName != null && mediaName != undefined && mediaName != "") {
            mediaUrl = getURL("originalimagecontainerRoot") + mediaName;
            /* $("#black").append('<img src="' + mediaUrl + '" alt="' + mediaName + '" width="500px" id="illustrationImage"/>');
            $("#bubble-black").css("text-align", "center");
            var illustrationImage = $("#illustrationImage");
            $("#bubble-black .bubcontent").imagesLoaded(function (illustrationImage, illustrationImage, illustrationImage) {
            var bubbleBlackHeight = illustrationImage.outerHeight(true);
            $("#bubble-black").css("height", bubbleBlackHeight);
            });*/
            $('#hostPic').html(mediaName);
            $('#actualImage').attr("src", mediaName);
            if ($("#hostpic").text() == '') {
                $("#backgroundpic").hide();
            } else {
                $("#backgroundpic").fadeOut(200);
            }
            onBlendTransparentImage(mediaName);
            if ($("#bgImg").text() != '') {
                $("#fullbgImg").hide();
